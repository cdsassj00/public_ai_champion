    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 overflow-hidden">
        ...
        >
          ...
          <div className="h-64 sm:h-80 md:h-auto md:w-[450px] shrink-0 relative bg-black overflow-hidden md:border-r border-white/5 flex items-center justify-center">
            {isRefining && (
          ...
          <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
            <div className="flex-1 overflow-y-auto p-6 md:p-16 lg:p-20 custom-scrollbar relative z-10">
              <div className="mb-10 md:mb-14">
                <div className={`inline-block px-4 py-1.5 md:px-5 md:py-2 mb-6 md:mb-10 text-[9px] md:text-[11px] font-black tracking-[0.4em] md:tracking-[0.6em] border ${details.border} ${details.color} uppercase bg-white/5 backdrop-blur-xl rounded-sm shadow-2xl drop-shadow-[0_0_8px_currentColor]`}>
                  {details.title} {details.icon}
                </div>
                <h2 className="text-3xl md:text-8xl font-black serif-title text-white mb-6 md:mb-8 tracking-tighter leading-tight break-keep drop-shadow-2xl">{champion.name}</h2>
                <div className="flex flex-wrap items-center gap-2 md:gap-3 text-white/60 text-[10px] md:text-lg font-light italic">
                  <span className="px-3 py-1 md:px-6 md:py-2.5 bg-white/[0.04] border border-white/10 rounded-full backdrop-blur-md">{champion.department}</span>
                  <span className="px-3 py-1 md:px-6 md:py-2.5 bg-white/[0.04] border border-white/10 rounded-full backdrop-blur-md">{champion.role}</span>
                </div>
              </div>
              ...
              <div className="space-y-12 md:space-y-24 pb-12">
                <section className="relative pl-8 md:pl-12">
                  ...
                  <div className="mb-4 md:mb-6">
                    <h4 className={`text-[9px] md:text-[11px] font-black uppercase tracking-[0.4em] ${details.color} opacity-60 flex items-center gap-3`}>
                      <span className="w-6 md:w-8 h-[1px] bg-current opacity-30"></span> AI MISSION & VISION
                    </h4>
                  </div>
                  {/* white-space: pre-wrap 적용으로 개행 지원 */}
                  <p className="text-lg md:text-4xl font-light italic text-white leading-[1.4] break-keep" style={{ whiteSpace: 'pre-wrap' }}>
                    "{champion.vision}"
                  </p>
                </section>
                ...
                {champion.achievement && (
                  <section>
                    <div className="mb-6 md:mb-8">
                       <h4 className={`text-[9px] md:text-[11px] font-black uppercase tracking-[0.4em] ${details.color} opacity-60 flex items-center gap-3`}>
                        <span className="w-6 md:w-8 h-[1px] bg-current opacity-30"></span> KEY ACHIEVEMENTS
                      </h4>
                    </div>
                    <div className={`p-6 md:p-14 bg-white/[0.02] border border-white/5 backdrop-blur-3xl relative rounded-sm shadow-inner overflow-hidden`}>
                      <div className={`absolute top-0 left-0 w-1.5 md:w-2 h-full ${details.accent} opacity-80 shadow-[0_0_20px_currentColor]`}></div>
                      {/* white-space: pre-wrap 적용으로 개행 지원 */}
                      <p className="text-sm md:text-2xl text-white/90 leading-relaxed font-light break-keep relative z-10" style={{ whiteSpace: 'pre-wrap' }}>
                        {champion.achievement}
                      </p>
                    </div>
                  </section>
            ...
            <div className="p-6 md:p-12 border-t border-white/5 bg-neutral-950/98 backdrop-blur-3xl flex flex-wrap items-center justify-end gap-3 md:gap-4 relative z-20">
              {isAuthorized && (
                <button 
                  onClick={handleManualRefine}
                  disabled={isRefining}
                  className={`mr-auto px-4 py-3 md:px-6 md:py-4 bg-white/5 border ${details.border} ${details.color} text-[9px] md:text-[11px] font-black uppercase tracking-widest hover:bg-white hover:text-black transition-all rounded-sm flex items-center gap-2 md:gap-3 shadow-xl`}
                >
                  <span className="text-sm animate-pulse">✨</span> {isRefining ? 'REFASHIONING...' : 'AI RECORD POLISH'}
                </button>
              )}
              
              <button onClick={() => handleManagementAction('DELETE')} disabled={isDeleting} className="px-4 py-3 md:px-8 md:py-4 border border-white/5 text-white/20 text-[9px] md:text-[11px] font-black uppercase tracking-widest hover:text-red-500 hover:border-red-500/30 transition-all rounded-sm">TERMINATE</button>
              <button onClick={() => handleManagementAction('EDIT')} className={`flex-1 md:flex-none px-6 py-4 md:px-14 md:py-5 border ${details.border} ${details.color} text-[10px] md:text-[12px] font-black uppercase tracking-[0.3em] md:tracking-[0.5em] hover:bg-white hover:text-black transition-all duration-500 rounded-sm shadow-2xl drop-shadow-[0_0_10px_currentColor]`}>EDIT RECORD</button>
            </div>
          </div>
        </motion.div>
      </div>
