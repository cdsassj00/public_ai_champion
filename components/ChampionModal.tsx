...
const ChampionModal: React.FC<ChampionModalProps> = ({ champion, onClose, onEdit, onDelete }) => {
  ...
  const verifyPin = () => {
    if (pinEntry === champion.passcode) {
      storageService.addOwnership(champion.id);
      if (pinAction === 'EDIT') onEdit(champion);
      else confirmDelete();
  ...
  const confirmDelete = async () => {
    if (window.confirm('이 기록을 명예의 전당에서 영구적으로 삭제하시겠습니까?')) {
      setIsDeleting(true);
      try {
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 30 }} className={`relative w-full max-w-5xl bg-neutral-950 border ${details.border} rounded-sm overflow-hidden shadow-[0_0_100px_rgba(0,0,0,1)] flex flex-col md:flex-row h-full max-h-[92vh] md:max-h-[85vh]`}>
            ...
            {showPinPrompt && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-[60] bg-black/95 backdrop-blur-3xl flex items-center justify-center p-6 text-center">
                <div className="max-w-xs w-full">
                  <h3 className="text-xl font-bold serif-title mb-2">본인 확인</h3>
                  <p className="text-[10px] text-white/40 uppercase tracking-[0.3em] mb-8">Enter Registry PIN to Unlock</p>
                  <input 
                    autoFocus
                    type="password"
                    maxLength={4}
                    ...
                    className="w-full bg-transparent border-b-2 border-yellow-500/30 text-center text-4xl tracking-[0.5em] py-4 focus:outline-none focus:border-yellow-500 transition-all font-mono mb-8"
                    placeholder="****"
                    onKeyDown={(e) => e.key === 'Enter' && verifyPin()}
                  />
                  <div className="flex gap-4">
                    <button onClick={() => setShowPinPrompt(false)} className="flex-1 py-3 border border-white/10 text-[10px] font-bold uppercase tracking-widest opacity-50 hover:opacity-100 transition-opacity">Cancel</button>
                    <button onClick={verifyPin} className="flex-1 py-3 bg-yellow-500 text-black text-[10px] font-black uppercase tracking-widest shadow-xl">Confirm</button>
                  </div>
                </div>
              </motion.div>
            )}
          ...
          <div className="h-56 sm:h-72 md:h-auto md:w-[350px] lg:w-[400px] shrink-0 relative bg-neutral-900 overflow-hidden md:border-r border-white/5">
            ...
            <div className="absolute bottom-8 left-8 right-8">
              <div className="flex items-center space-x-3 mb-2">
                ...
                <span className={`text-[10px] font-black tracking-[0.5em] uppercase ${details.color}`}>ID: {champion.id.split('_')[1]?.substring(0, 6)}</span>
              </div>
              <p className="text-white/40 text-[8px] font-black uppercase tracking-[0.3em]">National AI Innovation Archives</p>
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
            <div className="flex-1 overflow-y-auto p-8 md:p-14 custom-scrollbar relative z-10">
              ...
              <div className="space-y-12 md:space-y-16">
                ...
                <div className="grid grid-cols-2 gap-10 pt-10 border-t border-white/10">
                  <div className="group">
                    <span className="text-[9px] text-white/20 uppercase tracking-[0.4em] block font-black mb-2">Enrolled Date</span>
                    <span className="text-sm font-mono text-white/70 tracking-widest">{champion.registeredAt}</span>
                  </div>
                  <div className="group">
                    <span className="text-[9px] text-white/20 uppercase tracking-[0.4em] block font-black mb-2">Public Influence</span>
            ...
            <div className="p-6 md:p-8 border-t border-white/5 bg-neutral-950/80 backdrop-blur-2xl flex flex-wrap items-center justify-end gap-3 relative z-20">
              ...
              {champion.projectUrl && (
                <a href={champion.projectUrl} target="_blank" rel="noopener noreferrer" className="px-8 py-4 bg-white text-black text-[10px] font-black uppercase tracking-[0.4em] hover:bg-yellow-500 transition-all text-center rounded-sm">Verify Excellence</a>
              )}
            </div>
          </div>
        </motion.div>
