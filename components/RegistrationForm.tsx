...
const RegistrationForm: React.FC<RegistrationFormProps> = ({ onSuccess, editData }) => {
  ...
  const handleArtisticTransform = async () => {
    if (!formData.imageUrl) { alert('먼저 사진을 업로드해주세요.'); return; }
    ...
    try {
      ...
      console.error(error);
      alert('예술적 변환 중 오류가 발생했습니다.');
    } finally { setIsTransforming(false); }
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.imageUrl) { alert('사진을 등록하고 예술적으로 변환해보세요!'); return; }
    if (formData.passcode.length < 4) { alert('관리용 PIN 번호 4자리를 입력해주세요.'); return; }
    ...
    <div className="max-w-4xl mx-auto py-12 px-6">
      <div className="text-center mb-16">
        <h2 className="text-4xl md:text-5xl font-black serif-title mb-4 tracking-tighter uppercase">
          {isEditMode ? '프로필' : 'AI챔피언'} <span className="gold-text">{isEditMode ? '수정하기' : '등록하기'}</span>
        </h2>
        <p className="text-white/40 font-light italic break-keep px-4 text-xs md:text-sm leading-relaxed">
          {isEditMode ? '나의 업적을 최신 데이터로 갱신합니다.' : '"공공AI챔피언에 등록하여 나만의 브랜드 아이덴티티를 공유하세요"'}
        </p>
      </div>
        ...
        <div className="lg:col-span-5 flex flex-col space-y-6">
          <div className="relative group aspect-[4/5] bg-neutral-900 border-2 border-dashed border-white/10 rounded-sm overflow-hidden flex items-center justify-center">
            {isOptimizing ? (
              ...
              <img src={formData.imageUrl} className="w-full h-full object-cover transition-all duration-500" alt="Profile Preview" />
            ) : (
            ...
            <input type="file" ref={fileInputRef} onChange={handleImageUpload} className="absolute inset-0 opacity-0 cursor-pointer" accept="image/*" />
          </div>
          
          <button type="button" onClick={handleArtisticTransform} disabled={isTransforming || !formData.imageUrl || isOptimizing} className="w-full py-4 bg-white/5 border border-white/10 text-white font-bold text-[10px] tracking-widest uppercase hover:bg-yellow-500 hover:text-black transition-all disabled:opacity-30">
            {isTransforming ? '예술적 재창조 중...' : 'Nano Banana Pro 예술적 변환'}
          </button>

          {/* PIN 번호 입력창 - 금고 테마 */}
          <div className="p-6 bg-white/[0.02] border border-white/5 rounded-sm">
             <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/30 mb-3 block">관리용 PIN 번호 (4자리)</label>
             <input 
               required
               type="password" 
               maxLength={4}
               ...
               className="w-full bg-black/40 border border-white/10 p-3 text-center text-2xl tracking-[1em] focus:outline-none focus:border-yellow-500 focus:bg-black/60 transition-all font-mono"
               placeholder="****"
             />
             <p className="mt-3 text-[9px] text-white/10 font-medium italic break-keep leading-tight">이 번호는 프로필 수정 및 삭제 시 본인 확인을 위해 사용됩니다.</p>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 space-y-8 glass p-8 md:p-10 rounded-sm border-white/5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="flex flex-col space-y-2">
              ...
              <input required type="text" value={formData.department} onChange={e => setFormData({...formData, department: e.target.value})} className="bg-transparent border-b border-white/10 py-2 focus:outline-none focus:border-yellow-500 transition-colors text-lg" placeholder="부처/기관명" />
            </div>
          </div>

          <div className="flex flex-col space-y-2">
            <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/30">직위 / 주요 역할</label>
            <input required type="text" value={formData.role} onChange={e => setFormData({...formData, role: e.target.value})} className="bg-transparent border-b border-white/10 py-2 focus:outline-none focus:border-yellow-500 transition-colors text-sm" placeholder="예: 디지털전략과 팀장" />
          </div>
          ...
          <div className="flex flex-col space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={async () => {
                ...
                const sug = await suggestProfileContent(formData.name, formData.department, formData.role);
                setFormData(prev => ({...prev, achievement: sug}));
                setIsSuggesting(false);
              }} className="text-[9px] font-bold text-yellow-500 uppercase tracking-widest">{isSuggesting ? '...' : 'AI 추천'}</button>
            </div>
            <input type="text" value={formData.achievement} onChange={e => setFormData({...formData, achievement: e.target.value})} className="bg-transparent border-b border-white/10 py-2 focus:outline-none focus:border-yellow-500 transition-colors text-sm" placeholder="AI 도입 성과 등을 기록하세요" />
          </div>
          ...
          <div className="flex flex-col space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={async () => {
                if (!formData.vision) return alert('내용을 먼저 입력하세요.');
                setIsPolishing(true);
                const pol = await polishVision(formData.name, formData.department, formData.vision);
                setFormData(prev => ({...prev, vision: pol}));
                setIsPolishing(false);
              }} className="text-[9px] font-bold text-yellow-500 uppercase tracking-widest">{isPolishing ? '...' : 'Gemini AI 교정'}</button>
            </div>
            <textarea required rows={3} value={formData.vision} onChange={e => setFormData({...formData, vision: e.target.value})} className="bg-white/5 border border-white/5 p-4 focus:outline-none focus:border-yellow-500/30 transition-colors text-sm font-light leading-relaxed" placeholder="AI 챔피언으로서의 포부를 작성하세요." />
          </div>

          <button type="submit" disabled={isSubmitting || isOptimizing} className="w-full py-5 bg-yellow-500 text-black font-black uppercase tracking-[0.3em] text-[10px] md:text-xs hover:bg-yellow-400 transition-all disabled:opacity-50">
            {isSubmitting ? '진행 중...' : isEditMode ? '기록 업데이트' : '명예의 전당 헌액'}
          </button>
        </div>
      </form>
    </div>
